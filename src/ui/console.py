...
class ConsoleUI:
    ...
    def _get_priority_choice(self, current_priority: Optional[Priority] = None) -> str:
        ...
        print("2. Media")
        print("3. Alta")
        default_choice = str(list(Priority).index(current_priority) + 1) if current_priority else "2"
        return input(f"Seleccione la prioridad (1-3) [{default_choice}]: ").strip() or default_choice
    ...
    def _get_category_choice(self, current_category: Optional[str] = None) -> str:
        ...
        if cat_choice.isdigit():
            ...
            elif 1 <= choice_int <= len(categories):
    ...
    def _list_by_priority(self) -> None:
        ...
        if priority_choice in self.manager.PRIORITY_MAP:
            filtered_tasks = self.manager.get_priority_tasks(self.manager.PRIORITY_MAP[priority_choice])
